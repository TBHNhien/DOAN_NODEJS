...
// var sql = "CREATE TABLE AppUser (userID INT AUTO_INCREMENT PRIMARY KEY,fullname NVARCHAR(100),email VARCHAR(100),phoneNumber VARCHAR(11),sex NVARCHAR(5) CHECK (sex IN ('Nam', 'Nữ')),username VARCHAR(50),userPassword VARCHAR(100),userStatus INT,role VARCHAR(100),isdelete TINYINT(1) DEFAULT 0)";
...
//                 INSERT INTO AppUser (username, userPassword, fullname, email, phoneNumber, sex, userStatus, role)
//                 VALUES ("${username}", "${userPassword}", "${fullname}", "${email}", "${phoneNumber}", "${sex}", "${userStatus}","${role}")
//             `;
...
// Import và sử dụng hàm validator
const userValidator = validate.validator();



app.post('/app/register', userValidator, (req, res) => {
    const errors = validationResult(req);

    if (!errors.isEmpty()) {
        // Nếu có lỗi từ validator, trả về lỗi cho client
        return res.status(400).json({ errors: errors.array() });
    }

    // Lấy dữ liệu từ yêu cầu
    const { username, userPassword, fullname, email, phoneNumber, sex, userStatus, role } = req.body;

    // Kiểm tra xem tên người dùng đã tồn tại chưa
    const checkUserQuery = `SELECT * FROM AppUser WHERE email = ?`;
    ...
    con.query(checkUserQuery, [email], (err, result) => {
        ...
        if (result.length > 0) {
            ...
            con.query(insertUserQuery, [username, hashedPassword, fullname, email, phoneNumber, sex, userStatus, role], (err, result) => {
                if (err) {
                    console.error(err);
                    return res.status(500).json({ 'message': 'Internal Server Error' });
                }
...
// app.post('/app/checkLogin', async (req, res) => {
//     const email = req.body.email;
//     const userPass = req.body.userPassword;

//     try {
//         const user = await checkLogin(email, userPass);

//         if (user.err) {
//             // Xử lý khi có lỗi
//             res.status(400).json({ 'message': user.err });
//         } else {
