...
import com.example.appplanandnote.model.ErrorModel;
import com.example.appplanandnote.model.ErrorResponse;
            ...
            public void onResponse(Call<RegistrationResponse> call, Response<RegistrationResponse> response) {
                if (response.isSuccessful()) {
                    ...
                    if (registrationResponse != null) {
                        String successMessage = registrationResponse.getMessage();
                    ...
                    // Xử lý khi có lỗi từ server (mã trạng thái 400)
                    try {
                        // Parse JSON từ response.errorBody() để lấy thông báo lỗi
                        Gson gson = new Gson();
                        ErrorResponse errorResponse = gson.fromJson(response.errorBody().string(), ErrorResponse.class);

                        // Xử lý và hiển thị thông báo lỗi
                        List<ErrorModel> errors = errorResponse.getErrors();
                        StringBuilder errorMessage = new StringBuilder();
                        for (ErrorModel error : errors) {
                            errorMessage.append(error.getMsg()).append("\n");
                        }
                    ...
                    // Nếu không có thông báo lỗi cụ thể từ server, hiển thị thông báo mặc định
...
//                    }
            ...
            public void onFailure(Call<List<AppUser>> call, Throwable t) {
                ...
                Log.e("Retrofit", errorMessage + ": " + t.getMessage());
                Toast.makeText(context, errorMessage, Toast.LENGTH_LONG).show();
            }
        });
